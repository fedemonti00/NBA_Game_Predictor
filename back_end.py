...
d = pd.read_csv(r"fedemonti00\NBA_Game_predictor\dataset\Test_numeric.csv")
d_min = d.PTS.min()
d_max = d.PTS.max()
...
def predict_score(t1_player,t2_player):
    ...
    for i in range(10): #estrazione dei dataset
        dataset[i] = pd.read_csv(r'fedemonti00\NBA_Game_predictor\dataset\Dataset' + players[i].translate({ord(c): None for c in string.whitespace}) + '.csv')
